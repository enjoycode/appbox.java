...
import appbox.design.debug.proto.*;
...
public class JavaDebugServer implements DebugServer {
    ...
    private boolean matchesFile(BreakpointRequest b, Source source) {
        try {
            ...
            Log.warn("No source information for " + b.location());
            return false;
        }
    }
    ...
    public void attach(AttachRequestArguments req) {
        ...
        for (var string : req.sourceRoots) {
            ...
            if (!Files.exists(path)) {
                Log.warn(string + " does not exist");
            } else if (!Files.isDirectory(path)) {
                Log.warn(string + " is not a directory");
            } else {
    ...
    public void disconnect(DisconnectArguments req) {
        try {
            ...
            Log.warn("VM has already terminated");
        }
        vm = null;
    }
    ...
    public void next(NextArguments req) {
        ...
        if (thread == null) {
            Log.warn("No thread with id " + req.threadId);
            return;
        }
    ...
    private appbox.design.debug.proto.Thread[] asThreads(List<ThreadReference> ts) {
        var result = new appbox.design.debug.proto.Thread[ts.size()];
    ...
    private appbox.design.debug.proto.Thread asThread(ThreadReference t) {
        var thread = new appbox.design.debug.proto.Thread();
    ...
    private appbox.design.debug.proto.StackFrame[] asStackFrames(List<com.sun.jdi.StackFrame> fs) {
        var result = new appbox.design.debug.proto.StackFrame[fs.size()];
    ...
    private appbox.design.debug.proto.StackFrame asStackFrame(com.sun.jdi.StackFrame f) {
        var frame = new appbox.design.debug.proto.StackFrame();
    ...
    private Path findSource(Location l) throws AbsentInformationException {
        ...
        if (!warnedCouldNotFind.contains(relative)) {
            Log.warn("Could not find " + relative);
    ...
    public VariablesResponseBody variables(VariablesArguments req) {
        ...
        try {
            ...
            Log.warn(String.format("No visible variable information in %s", frame.location()));
