...
public class DebugAdapter {
    ...
    private static char read(InputStream client) {
        try {
            var c = client.read();
            if (c == -1) {
                Log.warn("Stream from client has been closed, throwing kill exception...");
    ...
    class ReceiveDebugClientEvents implements Runnable {
        ...
        private boolean kill() {
            ...
            try {
                ...
                Log.error("Failed to put kill message onto queue, will try again...");
                return false;
            }
        }
    }
    ...
    public void run() {
        ...
        while (true) {
            ...
            if (json == END_OF_STREAM) {
                Log.warn("Stream from client has been closed, exiting...");
                return;
            }
