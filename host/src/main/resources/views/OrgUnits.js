...
export default class OrgUnits extends Vue {
    ...
    const bizUnitBaseType = Long.fromString("-7018111290459553784");
    const groupBaseType = Long.fromString("-7018111290459553780");
    const empBaseType = Long.fromString("-7018111290459553788");

    const EmploeeView = sys.Views.EmployeeView
    ...
    /**根据类型获取节点的图标*/
    getIcon(data, node) {
        if (data.BaseType.eq(this.bizUnitBaseType)) {
            return 'fas fa-home'
        } else if (data.BaseType.eq(this.empBaseType)) {
            return 'fas fa-user'
        } else {
    ...
    onCurrentChanged(data, node) {
        ...
        if (data.BaseType.eq(this.empBaseType)) {
            this.orgUnitView = this.EmploeeView
        } else if (data.BaseType.eq(this.bizUnitBaseType)) {
    ...
    onCreateWorkgroup() {
        ...
        if (this.currentOrgUnit.BaseType.eq(this.empBaseType)) {
    ...
    mounted() {
        sys.Services.OrgUnitService.loadTree().then(res => {
            this.orgUnitNodes = res;
